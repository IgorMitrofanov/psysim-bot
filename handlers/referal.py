...
async def referral_handler(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession, achievement_system: AchievementSystem):
    ...
    if achievement_system:
        ...
        if referrals:
            await achievement_system.check_referral_achievements(
                db_user.id, 
...
async def my_referrals_handler(callback: types.CallbackQuery, session: AsyncSession, achievement_system: AchievementSystem):
    db_user = await get_user(session, telegram_id=callback.from_user.id)
    if not db_user:
    ...
    if achievement_system and referrals:
        await achievement_system.check_referral_achievements(
            db_user.id, 
            new_referrals_count=len(referrals))
